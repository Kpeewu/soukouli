...
@section('main-content')
    <div class="bg-body-light">
        <div class="content content-full">
            <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
                <h1 class="flex-sm-fill h3 my-2">Modifier l'Utilisateur</h1>
            </div>
        </div>
    </div>
    ...
    <div class="content">
        <div class="block block-rounded">
            <div class="block-header">
                <h3 class="block-title">Modifier {{ $user->username }}</h3>
            </div>
                ...
                <form action="{{ route('users.update', $user) }}" method="POST" id="form-user">
                    ...
                    <div class="block block-rounded block-bordered mb-4 pb-4">
                        ...
                        <div class="block-content">
                            ...
                            <div class="row mt-3" id="professeur-section" style="display: none;">
                                <div class="col-md-6">
                                    <div class="form-group mb-0">
                                        <label for="professeur_id"><i class="fa fa-chalkboard-teacher mr-1"
                                                aria-hidden="true"></i>Professeur associé</label>
                                        <select
                                            class="js-select2 form-control @error('professeur_id') is-invalid @enderror"
                                            id="professeur_id" name="professeur_id" style="width: 100%;"
                                            data-placeholder="Sélectionnez un professeur...">
                                            <option></option>
                                            @foreach ($professeursDisponibles as $professeur)
                                                <option value="{{ $professeur->id }}"
                                                    @selected((old('professeur_id') ?? $user->professeur?->id) == $professeur->id)>
                                        ...
                                        @error('professeur_id')
                                            <div class="invalid-feedback d-block">{{ $message }}</div>
                                        @enderror
                                        <small class="form-text text-muted">Le professeur doit être créé au
                                            préalable dans la liste des enseignants.</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="block-header block-header-default">
                            ...
                            <div class="block-options">
                                <button type="button" class="btn btn-sm btn-alt-primary" id="btn-generate-password">
                                    <i class="fa fa-magic mr-1" aria-hidden="true"></i>Générer un nouveau mot de passe
                                </button>
                            </div>
                        </div>
                        <div class="block-content">
    ...
    <script>
        // Ce bloc est rendu par @yield('content') avant que jquery.min.js soit charge
        // (voir layouts/app.blade.php) : sans ce report a DOMContentLoaded, l'appel a
        // jQuery() ci-dessous levait une erreur qui empechait tout le reste du script
        // (bascule et generation du mot de passe) de s'executer.
        document.addEventListener('DOMContentLoaded', function () {
            jQuery(function($) {
                var $roleField = $('#role');
                var $professeurSection = $('#professeur-section');
                var $professeurSelect = $('#professeur_id');

                function toggleProfesseurSection() {
                    var role = $roleField.val() || '';
                    if (role === 'professeur' || role.startsWith('professeur_')) {
                        $professeurSection.show();
                        $professeurSelect.attr('required', 'required');
                    } else {
                        $professeurSection.hide();
                        $professeurSelect.removeAttr('required');
                    }
                }

                // select2 declenche 'change' sur le <select> natif, donc ce handler
                // fonctionne aussi bien au clavier qu'a la souris.
                $roleField.on('change', toggleProfesseurSection);
                toggleProfesseurSection();
            });
